...
| 레이어 | 할 수 있는 것 | 금지 |
|--------|-------------|------|
| pages/ | 레이아웃, 훅 호출, 이벤트 핸들링 | 직접 fetch, 비즈니스 로직, 직접 table/input/modal 구현 |
| components/common/ | 공통 UI (SearchBar, DataTable, Dialog) | 비즈니스 로직 |
| components/{기능명}/ | 기능별 UI (Detail, Form) — **공통 컴포넌트 조합만** | 직접 table/modal 구현 |
